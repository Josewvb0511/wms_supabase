...
def _agrupar_saldos(df: pd.DataFrame, colunas_agrupar: list[str]) -> pd.DataFrame:
    ...
    if "custo_total_estoque" in df.columns:
        colunas_soma.append("custo_total_estoque")
...
def app():
    ...
    if "custo_unitario_cadastro" in df_prod.columns:
        custos = df_prod[["produto_codigo_merge", "custo_unitario_cadastro"]].copy()
        custos["produto_codigo_merge"] = custos["produto_codigo_merge"].astype(str).str.strip().str.upper()
        custos["custo_unitario_cadastro"] = pd.to_numeric(custos["custo_unitario_cadastro"], errors="coerce").fillna(0.0)
        custos = custos.drop_duplicates(subset=["produto_codigo_merge"], keep="last")
        saldo = saldo.merge(
            custos,
        ...
        saldo["custo_unitario"] = pd.to_numeric(saldo["custo_unitario_cadastro"], errors="coerce").fillna(0.0)
        saldo["custo_total_estoque"] = saldo["saldo_normal"] * saldo["custo_unitario"]
        saldo = saldo.drop(columns=[c for c in ["produto_codigo_merge", "custo_unitario_cadastro"] if c in saldo.columns])
    else:
        saldo["custo_unitario"] = 0.0
        saldo["custo_total_estoque"] = 0.0
    ...
    total_custo = float(df["custo_total_estoque"].sum()) if "custo_total_estoque" in df.columns else 0.0
    ...
    st.success(
        ...
        f"Custo estoque: R$ {total_custo:,.2f}"
    )